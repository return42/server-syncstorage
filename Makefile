...
PHONY += test
test:  $(ENV)
	MOZSVC_SQLURI=$(MOZSVC_SQLURI) $(ENV_BIN)/gunicorn --paste ./syncstorage/tests/tests.ini  --workers 1 --worker-class mozsvc.gunicorn_worker.MozSvcGeventWorker &\
	SERVER_PID=$$!; \
	sleep 2; \
	$(ENV_BIN)/python syncstorage/tests/functional/test_storage.py  http://localhost:5000 ; \
	kill $$SERVER_PID
